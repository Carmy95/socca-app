...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Information du Véhicule</h1>
                </div>
                <!-- /.col -->
                <div class="col-sm-6">
                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-md-4 col-lg-4 col-sm-6">
                                <a href="{{ route('cars.edit',$datas->id) }}" class="btn btn-primary">
                                    <i class="fas fa-edit"> </i> Modifier les Informations
                                </a>
                                @if (!$datas->chauffeurvehicule)
                                    <a href="{{ route('cars.assigne',$datas->id) }}" class="btn btn-primary">
                                        <i class="fas fa-user"> </i> Assigner un chauffeur
                                    </a>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-6" style="font-size: 35px; border-right: 1px solid #007bff">
                                    <p>Véhicule Immatriculé . {{ strtoupper($datas->immatriculation) }} </p>
                                    <p>de type : {{ strtoupper($datas->typevehicule->libelle) }}</p>
                                    <p>Appartenant à : {{ $datas->personne->sexe == 'H' ? 'Mr' : 'Mlle/Mme' }}. {{ strtoupper($datas->personne->nom) }} <span style="font-style: italic;">{{ ucwords($datas->personne->prenom) }}</span></p>
                                    @php
                                        if($datas->chauffeurvehicule)
                                        {
                                            $g = $datas->chauffeurvehicule->personne->sexe == 'H' ? 'Mr' : 'Mlle/Mme';
                                            $n = strtoupper($datas->chauffeurvehicule->personne->nom).' '.ucwords($datas->chauffeurvehicule->personne->prenom);
                                            $t = $g.' '.$n;
                                        }
                                    @endphp
                                    <p>Conduit par  : <span style="font-size: 25px; font-style: italic">{{ ($datas->chauffeurvehicule) ? $t : 'Pas encore assigné'  }}</span></p>
                                </div>
                                <div class="col-md-6" style="font-size: 35px; text-align:center;padding-top:3%; border-left: 1px solid #007bff">
                                    <img src="{{ asset('storage/'.$datas->photo.'') }}" alt="photo{{ $datas->immatriculation }}" style="border-radius: 100%; border: solid 4px #007bff;width:250px; height:250px; ">
                                </div>
                            </div>
                          </div>
                    </div>
