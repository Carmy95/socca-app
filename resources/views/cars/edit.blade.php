                                ...
                                <div class="card card-primary">
                                    ...
                                    <form action="{{ route('cars.update',$cars->id) }}" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control {{  $errors->first('imma','is-invalid') }}" value="{{ old('imma') ? old('imma') : $cars->immatriculation }}" id="imma" name="imma" placeholder="N° d'Immatriculation">
                                            {!! $errors->first('imma','<span style="color: red">:message</span>') !!}
                                        </div>
                                        <div class="form-group">
                                            <label for="type">Type du Véhicule</label>
                                            @php old('type') ? $t = old('type') : $t = $cars->typevehicule_id @endphp
                                            <select id="type" name="type" class="custom-select {{  $errors->first('type','is-invalid') }}">
                                                @foreach ($data as $datas)
                                                <option {{ $t == $datas->id ? 'selected' : '' }} value="{{ $datas->id }}">{{ strtoupper($datas->libelle) }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            @php old('proprio') ? $t = old('proprio') : $t = $cars->personne_id @endphp
                                            <select id="proprio" name="proprio" class="custom-select {{  $errors->first('proprio','is-invalid') }}">
                                                @foreach ($proprio as $item)
                                                <option {{ $t == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ strtoupper($item->nom).' '.ucwords($item->prenom) }}</option>
                                                @endforeach
                                            </select>
