                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody class="text-center">
                                    @if ($data->isEmpty())
                                        <tr>
                                            <td colspan="4">Aucune Commande Enregistré pour l'instant.</td>
                                        </tr>
                                    @else
                                        @php
                                            $t = 0;
                                        ...
                                        @foreach ($data as $item)
                                            <tr>
                                               ...
                                               <td>
                                                ...
                                                <td>
                                                    <a href="{{ route('commandes.edit',$item->id) }}" class="btn btn-primary" title="Modifier La commande">
                                                        <i class="fas fa-edit"> </i>
                                                    </a>
                                                </td>
                                                <td>
                                                    <form action="{{ route('commandes.destroy',$item->id) }}"
                                                        onsubmit="return confirm('Etre vous sûre de vouloir supprimer cette commande ?')"
                                                        method="post">
                                                        {{ csrf_field() }}{{ method_field('DELETE') }}
                                                        <button type="submit" class="btn btn-danger" title="Supprimer La commande"><i class="fas fa-trash"> </i></button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
